      ...
      <footer className="border-t border-[#123c2d]/12 px-5 py-12">
        <div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            ...
            <nav aria-label="Redes sociais" className="mt-6 flex gap-3">
              {socialLinks.map(([label, href, path]) => (
                <a
                  aria-label={label}
                  className="grid h-11 w-11 place-items-center rounded-full border border-[#C9A227]/35 text-[#C9A227] transition hover:bg-[#C9A227] hover:text-white"
                  href={href}
                  key={label}
                  rel="noreferrer"
                  target="_blank"
          ...
          <div className="text-sm text-[#52675e] md:text-right">
            <nav className="mb-6 grid gap-2 font-semibold text-[#C9A227] md:justify-items-end">
              {legalLinks.map(([label, href]) => (
                <Link className="hover:text-[#123c2d]" href={href} key={href}>
                  {label}
                </Link>
              ))}
              <span className="mt-3 text-[#C9A227]">{copy.legal.kvk}</span>
            </nav>
